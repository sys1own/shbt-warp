...
For the usual three-dimensional Alcubierre profile~\cite{Alcubierre:1994}
...
for every nontrivial localized profile.  The executable \path{StressEnergyAuditor}
samples random null and timelike vectors on the 3-D metric grid and reports the
minimum sampled energy densities
\begin{equation}
  \min_{\text{NEC}} T_{\mu\nu}k^\mu k^\nu = \SimOutputNEC,
  \qquad
  \min_{\text{WEC}} T_{\mu\nu}u^\mu u^\nu = \SimOutputWEC,
  \label{eq:numerical-energy-condition-minima}
\end{equation}
